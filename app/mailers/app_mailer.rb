class AppMailer < ApplicationMailer
  ...
  def new_user(user)
    @user = user
    mail :to => @user.email_address, :subject => "Welcome to VenMail"
  end

  def user_invite(user_invite, organization)
    @user_invite = user_invite
    @organization = organization
    mail :to => @user_invite.email_address, :subject => "Access the #{organization.name} organization on VenMail"
  end
  ...
  def password_reset(user, return_to = nil)
    @user = user
    @return_to = return_to
    mail :to => @user.email_address, :subject => "Reset your VenMail password"
  end
