class ReputationMonitorJob < Postal::Job
  ...
  def run_original_reputation_monitoring
    threshold = (Postal.config.general.reputation_block_threshold_percent || 8.0).to_f
    ...
    Server.where(suspended_at: nil).find_each(batch_size: 50) do |server|
      begin
        # Use 30-day window to match dashboard bounce rate calculation
        stats = server.message_db.statistics.get(:daily, [:outgoing, :bounces, :spam], Time.now, 30)
        totals = { outgoing: 0, bounces: 0, spam: 0 }
        stats.each do |date, stat|
          totals[:outgoing] += stat[:outgoing].to_f
          totals[:bounces] += stat[:bounces].to_f
          totals[:spam] += stat[:spam].to_f
        end
        ...
        Rails.logger.info "ReputationMonitorJob: Server #{server.id} - Outgoing: #{outgoing}, Bounces: #{bounces}, Bounce Rate: #{bounce_pct.round(2)}%"
        ...
        if bounce_pct >= credential_threshold
          ...
          Rails.logger.warn "ReputationMonitorJob: SUSPENDED all credentials for server #{server.id} due to #{bounce_pct.round(2)}% bounce rate"
        end

        # Note: Server suspension removed - only credentials are suspended at 8% threshold
      rescue => e
